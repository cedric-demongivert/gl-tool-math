...
describe('matrix.Matrix/*$ columns $*//*$ matrix_type $*/', function () {
  ...
  describe('#wrap', function () {
    it('allow to wrap an existing buffer into a matrix instance', function () {
      const buffer : /*$ matrix_buffer_type $*/ = new /*$ matrix_buffer_type $*/([
        /*% for row in print_rows(random_matrix(0)) : %*//*$ row $*//*% if loop.nextitem is defined %*/
        /*% endif %*//*% endfor %*/
      ])
  ...
  describe('#compute/*$ components[component] | upper $*/ComponentOfMultiplicationWithVector', function () {
    it('compute only one component of the vector that is a result of a multiplication of this matrix with a vector', function () {
      ...
      const vector : Vector = Vector.create(/*% for row in range(rows) : %*//*$ 1 if row == rows - 1 else row $*//*% if loop.nextitem is defined %*/, /*% endif %*//*% endfor %*/)
      const result : Vector = Vector.create(/*% for row in range(rows) : %*/0/*% if loop.nextitem is defined %*/, /*% endif %*//*% endfor %*/)
